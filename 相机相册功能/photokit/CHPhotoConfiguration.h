...
@class CHPhotoManager,CHPhotoModel;
...
/**
 ...
 demo1 里有示例（使用的是系统相机）
 */
//@property (copy, nonatomic) void (^shouldUseCamera)(UIViewController *viewController, CHPhotoManagerSelectedType cameraType, CHPhotoManager *manager);

/**
 相机拍照完成调用这个block 传入模型
 */
@property (copy, nonatomic) void (^useCameraComplete)(CHPhotoModel *model);
